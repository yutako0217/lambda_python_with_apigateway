...
def lambda_handler(event, context):
    ...
    if state == "stopped":
        ...
        res = ec2Client.instances.filter(InstanceIds=idList).start()
        return {"result": "success to start instance"}
    else:
        return {"result": "target instance is already running or pending"}